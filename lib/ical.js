...
// Pull a named parameter (e.g. CN, PARTSTAT, CUTYPE) out of an iCal property's params,
// stripping surrounding quotes. params look like ['CN="Jane Doe"', 'PARTSTAT=ACCEPTED'].
const getParam = (params, key) => {
  const hit = params.find((p) => p.toUpperCase().startsWith(key + '='));
  return hit ? hit.slice(key.length + 1).replace(/^"|"$/g, '') : '';
};
// ORGANIZER / ATTENDEE value is a mailto: URI; the display name is the CN param.
const parsePerson = (val, params) => {
  const email = String(val).replace(/^mailto:/i, '').trim();
  return { name: getParam(params, 'CN') || email, email, status: (getParam(params, 'PARTSTAT') || '').toLowerCase(), cutype: (getParam(params, 'CUTYPE') || 'INDIVIDUAL').toUpperCase() };
};
...
function parseIcs(text) {
  ...
  for (const line of lines) {
    ...
    else if (name === 'ORGANIZER') cur.organizer = parsePerson(val, params);
    else if (name === 'ATTENDEE') (cur.attendees = cur.attendees || []).push(parsePerson(val, params));
